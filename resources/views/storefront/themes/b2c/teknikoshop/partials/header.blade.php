...
<header class="ciak-header teknikoshop-header">
    ...
    <div class="ciak-nav-shell">
        <div class="ciak-nav-mobile">
            ...
            <a href="{{ route('storefront.home', $contextParams) }}" class="ciak-brand teknikoshop-brand" aria-label="{{ $publicStoreName }} catalogo">
        ...
        <div class="ciak-nav-desktop ciak-shell">
            <nav class="ciak-nav-side ciak-nav-side-left teknikoshop-nav-collections" aria-label="{{ __('themes_b2c.ciak.main_categories') }}">
                @foreach($leftCategories as $category)
                    @include('storefront.themes.b2c.teknikoshop.partials.header-category', ['category' => $category])
                @endforeach
            </nav>
            ...
            <div class="ciak-actions teknikoshop-actions">
                ...
                @endif
                <button type="button" class="ciak-icon-button teknikoshop-icon-button" data-ciak-search-toggle aria-label="{{ __('themes_b2c.ciak.search') }}"><i data-lucide="search"></i></button>
                <a href="{{ auth('customer')->check() ? route('storefront.wishlist.index', $contextParams) : route('storefront.login', $contextParams) }}" class="ciak-icon-button teknikoshop-icon-button" aria-label="{{ __('themes_b2c.ciak.favorites') }}"><i data-lucide="heart"></i></a>
                <a href="{{ auth('customer')->check() ? route('storefront.account.index', $contextParams) : route('storefront.login', $contextParams) }}" class="ciak-icon-button teknikoshop-icon-button" aria-label="{{ __('themes_b2c.ciak.account') }}"><i data-lucide="user-round"></i></a>
                <button type="button" class="ciak-icon-button teknikoshop-icon-button" data-bs-toggle="offcanvas" data-bs-target="#storefrontMinicart" aria-controls="storefrontMinicart" data-minicart-trigger aria-label="{{ __('themes_b2c.ciak.cart') }}"><i data-lucide="shopping-bag"></i><span class="ciak-count d-none" data-minicart-count-badge>0</span></button>
            </div>
        </div>
                ...
                <div class="storefront-search-shell" data-storefront-search-shell>
                    <div class="ciak-search-control storefront-search-control">
                        ...
                        <input
                            ...
                            aria-expanded="false"
                            aria-controls="teknikoshop-search-suggestions"
                    ...
                    <div id="teknikoshop-search-suggestions" class="storefront-search-suggestions d-none" role="listbox" aria-label="{{ __('themes_b2c.ciak.search_suggestions') }}" data-storefront-search-suggestions data-search-suggestions>
    ...
    <div class="offcanvas offcanvas-start ciak-mobile-menu" tabindex="-1" id="ciakMobileMenu">
        <div class="offcanvas-header">
            <a href="{{ route('storefront.home', $contextParams) }}" class="ciak-brand teknikoshop-brand" aria-label="{{ $publicStoreName }} home">
                @if(!empty($store?->logo_url))<img src="{{ $store->logo_url }}" alt="{{ $publicStoreName }}">@else<span>{{ $publicStoreName }}</span>@endif
            </a>
        ...
        <div class="offcanvas-body">
            <nav class="ciak-mobile-links">
                ...
                @foreach($navigationTree as $category)
                    @include('storefront.themes.b2c.teknikoshop.partials.header-category', ['category' => $category, 'mobile' => true])
                @endforeach
            </nav>
