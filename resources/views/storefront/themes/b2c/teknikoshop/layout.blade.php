...
<head>
    ...
    <title>{{ $seo['title'] ?? trim($__env->yieldContent('title', $store?->name ?? 'TEKNIKO')) }}</title>
    @include('storefront.base.partials.seo')
    <link rel="icon" type="image/jpeg" href="{{ media_url('favicon/tekniko-favicon.jpg') }}">
...
@endphp
<body
    id="teknikoshop-page-top"
    class="ciak-site teknikoshop-site storefront-page"
    data-storefront-layout="b2c-teknikoshop"
...
>
    @include('storefront.themes.b2c.teknikoshop.partials.header', ['contextParams' => $contextParams, 'agentContextId' => $agentContextId])
    <main class="ciak-main teknikoshop-main storefront-main">
    ...
    @include('storefront.themes.b2c.teknikoshop.partials.footer', ['contextParams' => $contextParams, 'agentContextId' => $agentContextId])
