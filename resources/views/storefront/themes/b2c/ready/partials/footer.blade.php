@php
    ...
    /*
     * Social READY configurati tramite config/services.php e .env.
     * I social senza URL non vengono mostrati.
     */
    $readySocials = collect([
        [
            'label' => 'Facebook',
            'url' => config('services.social.ready.facebook'),
            'icon_class' => 'fa-brands fa-facebook-f',
        ],
        [
            'label' => 'Instagram',
            'url' => config('services.social.ready.instagram'),
            'icon_class' => 'fa-brands fa-instagram',
        ],
        [
            'label' => 'TikTok',
            'url' => config('services.social.ready.tiktok'),
            'icon_class' => 'fa-brands fa-tiktok',
        ],
    ])->filter(
        fn (array $social) => filled($social['url'] ?? null)
    )->values();
    ...
    if ($footerCollections->isEmpty()) {
        $footerCollections = $footerCategories
            ->filter(function ($category) {
                ...
                return str_contains($text, 'ready')
                    || str_contains($text, 'collez');
            })
            ->values();
    }
...
<footer class="intempo-b2c-footer ready-footer">
    <div class="intempo-b2c-service-row intempo-b2c-shell">
        <a href="{{ route('storefront.catalog.index', $contextParams) }}">
            ...
            <span>
                ...
                <small>Prodotti Ready per outdoor e tempo libero</small>
            </span>
        </a>
        ...
        <a href="{{ route('storefront.store-locator.index', $contextParams) }}">
            ...
            <span>
                ...
                <small>Trova il rivenditore più vicino</small>
            </span>
        </a>
        ...
        <a href="{{ auth('customer')->check()
            ...
            <span>
                ...
                <small>Ordini, account e preferiti</small>
            </span>
        </a>
    ...
    <div class="intempo-b2c-footer-main intempo-b2c-shell">
        <div class="intempo-b2c-footer-brand">
            ...
            <p>
                Be smart, be ready. Accessori pratici per sport, outdoor,
            ...
            @if($readySocials->isNotEmpty())
                <div
                    class="footer-socials ready-footer-socials"
                    aria-label="Canali social Ready"
                >
                    @foreach($readySocials as $social)
                        <a
                            href="{{ $social['url'] }}"
                            target="_blank"
                            rel="noopener noreferrer"
                            class="footer-social-link ready-footer-social-link"
    ...
    <div class="intempo-b2c-footer-bottom intempo-b2c-shell">
        ...
        <nav
            class="intempo-b2c-footer-legal-links"
