...
class ShippingRuleUpdateRequest extends FormRequest
{
        ...
        $validator->after(function ($validator) {
            ...
            if ($type === 'free_over' && !$this->filled('min_amount')) {
                $validator->errors()->add(
                    'min_amount',
                    'Per la regola free_over la soglia minima è obbligatoria.'
                );
            }
            ...
            if (in_array($type, ['fixed', 'table'], true) && !$this->filled('amount')) {
                $validator->errors()->add(
                    'amount',
                    'Per questa regola il costo spedizione è obbligatorio.'
                );
            }
            ...
            if ($type === 'table' && !$this->filled('weight_from')) {
                $validator->errors()->add(
                    'weight_from',
                    'Per la regola table il peso minimo è obbligatorio.'
                );
            }
        });
    }
    ...
    public function messages(): array
    {
        return [
            ...
            'weight_from.min' => 'Il peso minimo deve essere maggiore o uguale a 0.',
            'min_amount.numeric' => 'La soglia minima deve essere numerica.',
            'min_amount.min' => 'La soglia minima deve essere maggiore o uguale a 0.',
            ...
            'amount.min' => 'L\'importo deve essere maggiore o uguale a 0.',
